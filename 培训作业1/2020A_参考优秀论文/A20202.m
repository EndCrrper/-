clc; clear;  %问题二
global F v T xm
F=[25,182,203,237,254,25];L=50+30.5*11+5*10; 
xm =[6.677003439269690e-04;2.999997701878186e+04;8.120630727730775e-04;1.499997442761376e+03;9.300295943899003e-04;1.389867556248244e+03;8.343291166559698e-04;6.898624109710258e+02;5.307244068332524e-04;1.265962465478697e+03];
%xm = [6.680738508034592e-04;2.665015993703618e+04;8.162196029159215e-04;1.199766752638612e+03;9.827794831794097e-04;8.065131954728832e+02;8.362762276929443e-04;7.385816657683414e+02;5.271121937960067e-04;1.363617464878294e+03];
...
t1=zeros(41,1);t2=t1; umax=t1; kmax=t1; 
for i=0:40
    v=1+i/60;
    T=L/v;
    u=heat(xm);
    ...
    u=u(u>=150);
    u=u(u<=190);
    t1(i+1)=(length(u)-1)*0.5;   
end

figure
ax1=subplot(2,2,1);ax2=subplot(2,2,2);
ax3=subplot(2,2,3);ax4=subplot(2,2,4);
plot(ax1,60:100,kmax);
title(ax1,'斜率的最大值与过炉速度的关系');
xlabel(ax1, '过炉速度（cm/min）');ylabel(ax1,'斜率');

plot(ax4,60:100,umax);
title(ax4,'温度峰值与过炉速度的关系');
xlabel(ax4, '过炉速度（cm/min）');ylabel(ax2,'温度（℃）');

plot(ax2,60:100,t1);
title(ax2,'升温期温度在150-190℃的时长与过炉速度的关系');
xlabel(ax2, '过炉速度（cm/min）');ylabel(ax3,'时间(s)');

plot(ax3,60:100,t2);
title(ax3,'温度在217℃的时长与过炉速度的关系');
xlabel(ax3, '过炉速度（cm/min）');ylabel(ax4,'时间(s)');
...
global xm
    ...
    s=s(s<=190);
    t1=length(s)*0.5;   
    y=(umax<=250)*(umax>=240)*(t2>=40)*(t2<=90)*(kmax<=3)*(t1<=120)*(t1>=60);
end