...
(2)~Crank--Nicolson矩阵预分解技术，将单步求解从$O(n^2)$降为$O(n^2)$;
...
\caption{主要符号说明}
\label{tab:symbols}
\begin{tabularx}{\textwidth}{l>{\raggedright\arraybackslash}p{7cm}l}
\toprule
...
\caption{炉内温度分布各段分界点}
\label{tab:oven}
\begin{tabularx}{\textwidth}{c>{\centering\arraybackslash}p{4.5cm}Y}
\toprule
...
预分解策略将每步计算从求解三对角方程组的$O(n)$优化为矩阵-向量乘法的$O(n^2)$，
由于矩阵$\mathbf{C}$在给定参数下不变，可在各段开始时一次性计算，后续时间步直接复用。
...
\midrule
小温区1$\sim$5温度 $T_{1-5}$ & $178.1^{\circ}\mathrm{C}$ & $[165,185]$ \\
小温区6温度 $T_6$           & $194.3^{\circ}\mathrm{C}$ & $[185,205]$ \\
小温区7温度 $T_7$           & $225.9^{\circ}\mathrm{C}$ & $[225,245]$ \\
小温区8$\sim$9温度 $T_{8-9}$& $265.0^{\circ}\mathrm{C}$ & $[245,265]$ \\
传送带速度 $v$              & $90.90\;\mathrm{cm/min}$ & $[65,100]$ \\
\midrule
\textbf{最小面积}           & \textbf{391.6 $^{\circ}\mathrm{C}\!\cdot\!\mathrm{s}$} & --- \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
问题3 (min面积)     & 392.5 & 0.1833 & ---  & ---  & 180.5 & 187.6 & 227.7 & 265.0 \\
问题4 (min对称性)  & 409.1 & \textbf{0.1803} & 0.0711 & 0.1803 & 179.4 & 198.6 & 226.5 & 265.0 \\
\bottomrule
\end{tabularx}
\end{table}
...
速度扫描数据揭示了清晰的单调规律：
速度每增加$5\;\mathrm{cm/min}$，峰值温度下降约$1.1^{\circ}\mathrm{C}$，$>$$217^{\circ}\mathrm{C}$时间缩短约$7\;\mathrm{s}$，温度变化斜率增加约$0.015^{\circ}\mathrm{C}/\mathrm{s}$。
